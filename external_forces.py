...
class SoftRobotForces(NoForces):
    """
    This class applies constant forces on the endpoint nodes.
    ...
    def __init__(self, forces, ramp_up_time=0.0):
        """
        ...
        """
        super(SoftRobotForces, self).__init__()
        self.forces=forces
        assert ramp_up_time >= 0.0
        self.ramp_up_time = ramp_up_time
    ...
    def apply_forces(self, system, time=0.0):
        ...
        system.external_forces[..., 10] += self.forces[0] * factor
        system.external_forces[..., 20] += self.forces[1] * factor
        system.external_forces[..., 30] += self.forces[2] * factor
        system.external_forces[..., 40] += self.forces[3] * factor
        system.external_forces[..., 50] += self.forces[4] * factor
